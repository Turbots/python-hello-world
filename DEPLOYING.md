...
```
pack build python-hello-world --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.0.10
```

Where `python-hello-world` is the name of your runnable function image, later used by Docker.
...
```
docker run -it --rm -p 8080:8080 python-hello-world
```
...
```
tanzu apps workload create python-hello-world -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/python-hello-world-source \
  --type web
```
...
```
tanzu apps workload get python-hello-world
```
